...
export const NeuralKnot: FC<NeuralKnotProps> = ({
    ...
    return (
        <div className={`relative rounded-full flex items-center justify-center flex-shrink-0 ${sizeMap[size]} ${className}`}>
            ...
            {state === 'idle' && (
                <div className="absolute inset-0 rounded-full animate-[pulse_3s_ease-in-out_infinite] opacity-40 ring-2 ring-indigo-300" />
            )}
            {state === 'speaking' && (
                <div className="absolute inset-0 rounded-full ring-4 ring-indigo-400/50 animate-pulse" />
            )}
            {state === 'thinking' && (
