...
export async function evaluateInterview(
    input: MasterJudgeInput
): Promise<MasterJudgeOutput> {
    ...
    try {
        ...
        const result = await provider.generateJson<MasterJudgeOutput>(
            `${MASTER_JUDGE_SYSTEM_PROMPT}\n\n${userPrompt}`,
            {
...
function buildUserPrompt(input: MasterJudgeInput): string {
...
function createErrorResult(startTime: number, errorMsg: string): MasterJudgeOutput {
    return {
        textScore: 0,
        voiceScore: 0,
        videoScore: 0,
...
// ============================================================================
// QUICK CONTENT JUDGE (for per-answer instant feedback)
// ============================================================================
...
export async function quickJudgeAnswer(
    input: QuickJudgeInput
): Promise<QuickJudgeOutput> {
    const startTime = Date.now();
...
Answer: "${answer}"`;
    ...
    try {
        const provider = LLMFactory.getProvider();
        const result = await provider.generateJson<QuickJudgeOutput>(prompt, {
            temperature: 0.2,
            maxOutputTokens: 256,
        });
