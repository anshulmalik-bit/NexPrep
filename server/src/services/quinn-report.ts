...
${evalData ? `[PRE-SCORED: Score ${evalData.score}, Flags: ${evalData.flags?.join(', ') || 'None'}]` : ''}`;
    ...
    try {
        ...
        if (!report.evaluations || !Array.isArray(report.evaluations) || report.evaluations.length === 0) {
            console.error("LLM returned empty evaluations. Report:", JSON.stringify(report));
            throw new Error("Invalid evaluations generated by AI");
        }
        ...
        return {
            overallScore: 0,
            summary: "Interview completed. Detailed AI analysis unavailable due to service interruption. Please try again later.",
            skillMatrix: [
            ...
            evaluations: answers.map((_, i) => {
                ...
                return {
                    ...
                    starRating: pre ? 3 : 1,
                    feedback: "AI Analysis Temporarily Unavailable.",
                    strength: pre ? pre.strengths[0] : "Answer recorded",
                    weakness: pre ? pre.weaknesses[0] : "Analysis failed",
                    improvedSample: "N/A"
                };
            })
        };
